...
O sistema MUST permitir que o usuário autenticado crie uma carteira informando um nome não vazio e, opcionalmente, uma descrição. O sistema MUST associá-la a esse usuário e atribuir identificador e data de cadastro.

#### Scenario: Cadastro válido
- **WHEN** o usuário autenticado envia `POST /carteiras` com nome válido
- **THEN** o sistema persiste uma carteira de sua propriedade e retorna `201 Created` com sua representação DTO
...
- **WHEN** o usuário envia nome já utilizado em outra carteira sua, ignorando diferenças de maiúsculas/minúsculas e espaços externos
- **THEN** o sistema retorna `409 Conflict` sem criar uma segunda carteira

### Requirement: Consultar carteiras
O sistema MUST listar somente as carteiras do usuário autenticado e permitir consultar por ID somente uma carteira que lhe pertença, retornando somente DTOs.

#### Scenario: Listagem
- **WHEN** o usuário autenticado envia `GET /carteiras`
- **THEN** o sistema retorna `200 OK` com sua coleção de carteiras

#### Scenario: Consulta existente
- **WHEN** o usuário autenticado envia `GET /carteiras/{id}` para uma carteira sua
...
O sistema MUST persistir cada carteira com owner obrigatório, nome normalizado para comparação, descrição opcional e data de cadastro, protegendo no banco a unicidade do nome normalizado dentro de cada owner.

#### Scenario: Unicidade concorrente
- **WHEN** duas tentativas concorrentes do mesmo usuário criam o mesmo nome normalizado
