...
Esta capacidade MUST limitar seus próprios endpoints ao cadastro e consulta básica, mas a carteira MAY ser referenciada por operações registradas pela capacidade `operation-management` e MAY expor consultas derivadas pela capacidade `portfolio-position-calculation`. A carteira não deve executar cálculos dentro de sua entidade nem alterar dados cadastrais durante essas consultas.
